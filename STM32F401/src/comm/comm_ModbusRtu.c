...
//  MasterPacketConstructor(hPort, DVIBR_SLAVE_ID, MB_READ_REGS, DVIBR_DATA_ADDR, DVIBR_REGS_COUNT);
  ...
  switch(Packet->Request)//формирование
  {
    ...
    case MB_READ_INPUT_REGS: ReadRegsRequest(hPort); break;
...
__inline void MasterConfirmation(TMbPort *hPort)
{
  ...
  else switch(Packet->Request)
  {
    ...
    case MB_READ_INPUT_REGS:   ReadRegsConfirmation(hPort); break;
...
void ModbusTx(TMbPort *hPort)
{
  ...
  if ((Frame->Data - Frame->Buf) < Frame->TxLength)     //передает если указатель ещё не больше размера сообщения
    USART_SendData(USART2, (uint8_t) *Frame->Data++);   //передача байта буфера фрейма
  else                                                  //иначе запускает таймер постамбулы
    StartMbTimer(&Frame->TimerPost);    // = Timer3_5, просто разные сбросы для приема и передачи	
